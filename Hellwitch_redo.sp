...
enum struct HellPortal
{
	...
	void HellPortal_Hint()
	{
		...
		DispatchKeyValue(entity, "hint_caption", "Cehennem Geçidi'nin Kapısı Açıldı, Saldırıya Hazır OL!");
...
public void OnEntityCreated(int entity, const char[] classname)
{
	...
	if(classname[0] == 'i' && (g_bCatchSpawning || AnyPortalsExist()))
	{
...
public Action HellSpawnSound(int clients[64], int &numClients, char sample[PLATFORM_MAX_PATH], int &entity, int &channel, float &volume, int &level, int &pitch, int &flags)
{
	if(g_bCatchSpawning)
	{
		if(StrEqual(sample, "common/null.wav", false))
			return Plugin_Continue;
	...
	{
		if(sample[0] == 'n' && StrEqual(sample, "npc/witch/voice/die/female_death_1.wav", false))
			return Plugin_Handled;
...
bool SpawnLowerHellMob(ZombieSpawns Type)
{
	...
	g_vecSpawnOriginHack = NULL_VECTOR;// medic
	switch(Type)
	{
		case ZombieSpawns_Tank:
		{
