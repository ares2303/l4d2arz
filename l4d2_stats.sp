...
Handle g_hGlowOverlayColor = null;
Handle g_hGlowOverlayGlow = null;
...
{
	if (0 < ok <= L4D_MAXPLAYERS && IsClientConnected(ok) && IsClientInGame(ok))
	{
		return 1;
	}
	return 0;
...
public void OnPluginStart()
{
	...
	g_hGlowOverlayColor = RegClientCookie("glow_overlay_color", "Glow overlay color settings", CookieAccess_Private);
	g_hGlowOverlayGlow = RegClientCookie("glow_overlay_glow", "Glow overlay glow settings", CookieAccess_Private);
}
...
public void RanktyConnect(int client)
{
	...
	if (IsClientInGame(client) && !IsFakeClient(client))
	{
		if (GetClientTeam(client) == TEAM_SURVIVORS)
		{
			...
			{
				if (ClientPoints[client] >= 800000)
				{
					SetEntityRenderColor(colorEnt, 0, 0, 0, 255);
				}
				else if (ClientPoints[client] >= 640000)
				{
					SetEntityRenderColor(colorEnt, 255, 97, 3, 255);
				}
				else if (ClientPoints[client] >= 320000)
				{
...
void GrantPlayerColor(int client)
{
	...
	if (hm_stats_colors.IntValue < 1 || GetClientTeam(client) != 2) return;
	{
		if (IsValidClient(client) || IsClientConnected(client) || IsClientInGame(client) || IsPlayerAlive(client))
		{
			if (IsFakeClient(client))
			{
		...
		if (hm_stats_colors.IntValue == 1)
		{
			...
			{
				if (ClientRank[client] < 4) SetEntityRenderColor(colorEnt, 160, 32, 240, 255);
				else if (ClientRank[client] < 11) SetEntityRenderColor(colorEnt, 255, 0, 0, 255);
				else if (ClientRank[client] < 21) SetEntityRenderColor(colorEnt, 0, 0, 255, 255);
				else if (ClientRank[client] < 31) SetEntityRenderColor(colorEnt, 255, 255, 0, 255);
				else if (ClientRank[client] < 41) SetEntityRenderColor(colorEnt, 0, 255, 0, 255);
		...
		if (hm_stats_colors.IntValue == 2)
		{
			...
			{
				if (ClientPoints[client] > 5000 && ClientPoints[client] < 10001) SetEntityRenderColor(colorEnt, 173, 255, 47, 255);
				else if (ClientPoints[client] > 10000 && ClientPoints[client] < 20001) SetEntityRenderColor(colorEnt, 255, 255, 0, 255);
				else if (ClientPoints[client] > 20000 && ClientPoints[client] < 40001) SetEntityRenderColor(colorEnt, 0, 0, 255, 255);
				else if (ClientPoints[client] > 40000 && ClientPoints[client] < 80001) SetEntityRenderColor(colorEnt, 0, 139, 0, 255);
				else if (ClientPoints[client] > 80000 && ClientPoints[client] < 160001) SetEntityRenderColor(colorEnt, 102, 25, 140, 255);
				else if (ClientPoints[client] > 160000 && ClientPoints[client] < 320001) SetEntityRenderColor(colorEnt, 255, 104, 240, 255);
				else if (ClientPoints[client] > 320000 && ClientPoints[client] < 640001) SetEntityRenderColor(colorEnt, 255, 0, 0, 255);
				else if (ClientPoints[client] > 800000) SetEntityRenderColor(colorEnt, 0, 0, 0, 255);
				else if (ClientPoints[client] > 640000) SetEntityRenderColor(colorEnt, 255, 97, 3, 255); 
			}
			else
			{
				SetEntityRenderColor(colorEnt, 255, 255, 255, 255);
...
int GetRankColorTarget(int client)
{
	...
	if (g_hGlowOverlayColor != null && AreClientCookiesCached(client))
	{
		char sColor[32];
		GetClientCookie(client, g_hGlowOverlayColor, sColor, sizeof(sColor));
		if (sColor[0] != '\0') return 0;
	}

	if (g_hGlowOverlayGlow != null && AreClientCookiesCached(client))
	{
		char sGlow[32];
		GetClientCookie(client, g_hGlowOverlayGlow, sGlow, sizeof(sGlow));
		if (sGlow[0] != '\0') return 0;
	}
