...
static int iGlow[MAXPLAYERS+1][2];
static bool bThirdPerson[MAXPLAYERS+1] = {false, ...};
static bool bDelayHide[MAXPLAYERS+1] = {false, ... };
...
{
	bDelayHide[iClient] = false;
	iGlow[iClient][GLOWCOLOR] = 0;
	iGlow[iClient][GLOWTYPE] = 0;
}
	...
	switch(GetClientTeam(iOwner)) 
	{
		case 2: 
		{
			...
			if(!IsSurvivorThirdPerson(iClient, false))
			{
				...
				dp.WriteCell(EntIndexToEntRef(iEntity));
				RequestFrame(OnNextDelayHide, dp);

				return Plugin_Continue;
			}
			else
			{
				if (iGlow[iClient][GLOWCOLOR] == 0 && iGlow[iClient][GLOWTYPE] == 0)
				{
					iGlow[iClient][GLOWCOLOR] = GetEntProp(iEntity, Prop_Send, "m_glowColorOverride", 0);
					iGlow[iClient][GLOWTYPE] = GetEntProp(iEntity, Prop_Send, "m_iGlowType", 0);
				}

				bDelayHide[iClient] = true;
				return Plugin_Continue;
			}
		}
		case 3: 
...
void OnNextDelayHide(DataPack dp)
{
	dp.Reset(false);
	int iClient = dp.ReadCell();
	int iEntity = EntRefToEntIndex(dp.ReadCell());
	delete dp;
	...
	{
		if (iGlow[iClient][GLOWCOLOR] > 0 && iGlow[iClient][GLOWTYPE] == 3)
		{
			SetEntProp(iEntity, Prop_Send, "m_glowColorOverride", iGlow[iClient][GLOWCOLOR]);
			SetEntProp(iEntity, Prop_Send, "m_iGlowType", iGlow[iClient][GLOWTYPE]);
		}
	}
